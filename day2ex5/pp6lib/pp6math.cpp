...
void sort(double& a, double& b){
  ...
  if( a < b )  swap (a , b); // swap if next element is larger
  
}

void checksort(double a, double b, int& k){

  if(a >=  b) k++; // If entry is larger than next one, increment k
...
void indexsort(double& a, double& b, int& j1, int& j2){
